 ...
 * Target field
 */
function ed11y_checkRoots_field() {
 ...
 * Video field
 */
function ed11y_videoContent_field() {
 ...
 * Audio field
 */
function ed11y_audioContent_field() {
 ...
 * embeddedContent 
 */
function ed11y_embeddedContent_field() {
...
?>
    ...
    <p id="extra_props_description">
        <?php 
            ...
            echo sprintf( esc_html__( '%1$s', 'ed11y-wp' ), $link );
        ?>
    </p> 
<?php
}
...
function ed11y_plugin_settings_render_page() { ?>
    ...
    <div class="wrap">
        <h1><?php esc_html_e('Editoria11y - Advanced Settings', 'ed11y-wp'); ?></h1>
                    ...
                    <div class="inside">
                        ...
                        <ul>
                            <li>
                                ...
                                <?php echo Editoria11y_WP::ED11Y_VERSION; ?>
                            </li>
                            <li>
                                <strong><?php esc_html_e( 'Plugin', 'ed11y-wp' ); ?>:</strong> 
                                <?php echo Editoria11y_WP::WP_VERSION; ?> 
...
function ed11y_plugin_settings_validate($settings) {
    ...
    $settings['ed11y_enable'] = (isset(
        $settings['ed11y_enable']) && 1 == $settings['ed11y_enable'] ? 1 : 0);

    $settings['ed11y_checkRoots'] = strtr(
        sanitize_text_field($settings['ed11y_checkRoots']), $targetRemove);

    /* Exclusions */
    $settings['ed11y_ignore_elements'] = strtr(
        sanitize_text_field($settings['ed11y_ignore_elements']), $remove);

    $settings['ed11y_link_ignore_strings'] = strtr(
        sanitize_text_field($settings['ed11y_link_ignore_strings']), $remove);

    $settings['ed11y_links_to_flag'] = strtr( 
        sanitize_text_field($settings['ed11y_links_to_flag']), $remove);
    ...
    /* Video */
    $settings['ed11y_videoContent'] = preg_replace($specialChars, '', 
        sanitize_text_field($settings['ed11y_videoContent']));

    /* Audio */
    $settings['ed11y_audioContent'] = preg_replace($specialChars, '', 
        sanitize_text_field($settings['ed11y_audioContent']));

    /* Data Visualizations */
    $settings['ed11y_embeddedContent'] =  preg_replace($specialChars, '', 
        sanitize_text_field($settings['ed11y_embeddedContent']));

    /* Don't run Editoria11y */
    $settings['ed11y_no_run'] = strtr( 
        sanitize_text_field($settings['ed11y_no_run']), $targetRemove);

    /* Advanced props */
    $settings['ed11y_extra_props'] =  preg_replace($specialChars, '', 
